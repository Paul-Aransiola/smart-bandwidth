...
class AlertRule(Base):
    ...
    # Rule configuration
    metric: Mapped[AlertMetric] = mapped_column(Enum(AlertMetric), nullable=False, index=True)
    ...
    time_window_minutes: Mapped[int] = mapped_column(
        Integer, default=5, nullable=False, comment="Time window in minutes for evaluation"
    )
    ...
    # Relationships
    device: Mapped["Device"] = relationship("Device", back_populates="alert_rules", lazy="selectin")
    alerts: Mapped[list["Alert"]] = relationship(
        "Alert", back_populates="rule", cascade="all, delete-orphan"
    )

    __table_args__ = (Index("idx_alert_rules_enabled_metric", "is_enabled", "metric"),)
...
class Alert(Base):
    ...
    notifications_sent: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="JSON array of notification delivery records"
    )
